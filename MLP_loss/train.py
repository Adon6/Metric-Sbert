...
model_save_path = (
    "output/L_" + model_name.replace("/", "-") + "-" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
)

checkpoint_save_path = (
    "output/L_" + model_name.replace("/", "-") + "-" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "/checkpoint"
)
...
dev_evaluator = MLPEvaluator.from_input_examples(
    dev_samples, 
...
test_model = MLPLoss.load(model_save_path)
